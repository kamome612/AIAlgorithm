...
void Enemy::Draw()
{
	...
	if (ImGui::RadioButton("RightHandMove", type_ == 0)) {
		type_ = 0;
	}
	if (ImGui::RadioButton("LeftHandMove", type_ == 1)) {
		type_ = 1;
	}

	if (ImGui::RadioButton("BFS", type_ == 3 && !isDijkstra_)) {
		isDijkstra_ = false;
		type_ = 3;
	}
	if (ImGui::RadioButton("Dijkstra", type_ == 3 && isDijkstra_)) {
		isDijkstra_ = true;
		type_ = 3;
	}
